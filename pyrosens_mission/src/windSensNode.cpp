...
WindSensNode::WindSensNode() : rclcpp::Node("wind_sensor_node") {
    log_samples_ = this->declare_parameter<bool>("enable_sample_logging", false);
    ...
    param_cb_handle_ = this->add_on_set_parameters_callback(
      [this](const std::vector<rclcpp::Parameter> &params){
        ...
        for (const auto &p : params) {
          if (p.get_name() == "enable_sample_logging") {
            log_samples_ = p.as_bool();
          }
        }
        return result;
      });
...
void WindSensNode::onTestCloud(const sensor_msgs::msg::PointCloud2 & cloud) {
  ...
  for (; it_x != it_x.end(); ++it_x, ++it_y, ++it_z, ++it_u, ++it_v, ++i) {
    ...
    if (log_samples_) {
      RCLCPP_INFO(
        this->get_logger(),
        "Wind sample (%.2f, %.2f, %.2f) vel=(%.2f, %.2f)",
        p_msg.point.x, p_msg.point.y, p_msg.point.z,
        v_msg.vector.x, v_msg.vector.y);
    }
